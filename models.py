...
class Customer(Base):
    ...
    customer_id = Column(Integer(), primary_key=True, nullable=False, unique=True, autoincrement=True)
...
class Order(Base):
    __tablename__ = "order"
    order_id = Column(Integer(), primary_key=True, nullable=False, unique=True, autoincrement=True)
    order_date = Column(DateTime, default=datetime.now(timezone(offset=-timedelta(hours=4))))
...
class Addon(Base):
    __tablename__ = "addon"
    addon_id = Column(Integer(), primary_key=True, nullable=False, unique=True, autoincrement=True)
...
class CustomerOrder(Base):
    ...
    order_id = Column(Integer(), ForeignKey("order.order_id"), primary_key=True, nullable=False)
...
class OrderItem(Base):
    ...
    order_item_id = Column(Integer(), primary_key=True, nullable=False, unique=True, autoincrement=True)
...
class ItemMod(Base):
    ...
    order_item_id = Column(Integer(), ForeignKey("order_item.order_item_id"), primary_key=True, nullable=False)
    addon_id = Column(Integer(), ForeignKey("addon.addon_id"), primary_key=True, nullable=False)
...
class ManageResturantData:
    ...
    def new_food_item(self, session: Session, 
                         food_name: str,
                         food_price: int,
                         food_category: str,
        ...
        food_details = Food(food_name=food_name, 
    ...
    def update_food(self, session: Session, 
                          food_id: int,
                          **kwargs
        ...
        required = [column.name for column in inspect(Food).c if not column.name.endswith("_id")]
        if not any(i in kwargs for i in required):
            raise ValueError(f"Expected at least one attribute of Food entity.\nThese are:\n{'\n'.join(required)}")

        if any(attr in self.food_size_required for attr in kwargs) and not 'food_size' in kwargs:
            raise ResturantException("A size is required for this food type.")
    ...
    def new_addon_item(self, session: Session,
                          addon_name: str,
                          addon_type: str,
                          addon_price: int,
        ...
        addon_details = Addon(addon_name=addon_name,
    ...
    def update_addon(self, session: Session,
                     addon_id: int,
                     **kwargs
                     ) -> sqlalchemy.orm.query.Query:
        ...
        required = [column.name for column in inspect(Addon).c if not column.name.endswith("_id")]
        if not any(i in kwargs for i in required):
            raise ValueError(f"Expected at least one attribute of Addon entity.\nThese are:\n{'\n'.join(required)}")

        if any(attr in self.food_size_required for attr in kwargs) and not 'addon_size' in kwargs:
            raise ResturantException("A size is required for this addon type.")
    ...
    def remove_addon(self, session: Session, addon_id: int) -> sqlalchemy.orm.query.Query:
    ...
    def new_customer(self, session: Session,
                             customer_name: str,
        ...
        customer_details = Customer(customer_name=customer_name,
    ...
    def update_customer(self, session: Session,
                       customer_id: int,
                       **kwargs
                       ) -> sqlalchemy.orm.query.Query:
        """Updates the value of an attribute of a given customer."""
        entity = session.query(Customer).filter(Customer.customer_id == customer_id).first()
        if not entity:
            raise EntityNotFound("The customer does not exist in the database.")

        required = [column.name for column in inspect(Customer).c if not column.name.endswith("_id")]
        if not any(i in kwargs for i in required):
            raise ValueError(f"Expected at least one attribute of Customer entity.\nThese are:\n{'\n'.join(required)}")
    ...
    def new_order(self, session: Session,
                  order_date,
                  order_payment_method: str,
                  order_type: str) -> dict:
        """Adds order details to the database."""
        order_details = Order(order_date=order_date, 
    ...
    def update_order(self, session: Session,
                     order_id: int,
                     **kwargs):
        ...
        required = [column.name for column in inspect(Order).c if not column.name.endswith("_id")]
        if not any(i in kwargs for i in required):
            raise ValueError(f"Expected at least one attribute of Order entity.\nThese are:\n{'\n'.join(required)}")
    ...
    def new_customer_order(self, session: Session,
                            customer_id: int,
                            order_id: int):
          """Adds customer order details to the database."""
          customer_order_details = CustomerOrder(customer_id=customer_id,
                                                 order_id=order_id
                                                 )
    ...
    def remove_customer_order(self, session: Session, customer_id: int, order_id: int):
        ...
        entity = session.query(CustomerOrder).\
                    filter(CustomerOrder.customer_id == customer_id).\
                    filter(CustomerOrder.order_id == order_id).first()
        if not entity:
    ...
    def new_order_item(self, session: Session,
                       order_id: int,
                       food_id: int,
                       order_item_price: int) -> OrderItem:
        """Adds order item details to the database."""
        order_item_details = OrderItem(order_id=order_id,
                                       food_id=food_id,
    ...
    def update_order_item(self, session: Session,
                            order_item_id: int,
                            **kwargs) -> OrderItem:
        """Updates the value of an attribute of a given order item."""
        entity = session.query(OrderItem).filter(OrderItem.order_item_id == order_item_id).first()
        if not entity:
            raise EntityNotFound("The order item does not exist in the database.")

        required = [column.name for column in inspect(OrderItem).c if not column.name.endswith("_id")]
        if not any(i in kwargs for i in required):
            raise ValueError(f"Expected at least one attribute of OrderItem entity.\nThese are:\n{'\n'.join(required)}")
        ...
        update = session.query(OrderItem).\
    ...
    def new_item_mod(self, session: Session,
                        order_item_id: int,
                        addon_id: int,
                        item_mod_qty: int,
                        item_mod_price: int):
            """Adds item modification details to the database."""
            item_mod_details = ItemMod(order_item_id=order_item_id,
                                    addon_id=addon_id,
    ...
    def update_item_mod(self, session: Session,
                        order_item_id: int,
                        addon_id: int,
                        **kwargs):
            required = [column.name for column in inspect(ItemMod).c if not column.name.endswith("_id")]
            if not any(i in kwargs for i in required):
                raise ValueError(f"Expected at least one attribute of ItemMod entity.\nThese are:\n{'\n'.join(required)}")
    ...
    def remove_item_mod(self, session: Session, order_item_id: int, addon_id: int):
        ...
        entity = session.query(ItemMod).\
                    filter(ItemMod.order_item_id == order_item_id).\
                    filter(ItemMod.addon_id == addon_id).first()
        if not entity:
            raise EntityNotFound("The item modification does not exist in the database.")
        session.delete(entity)
        session.commit()
        return entity
...
class ViewResturantData:
    ...
    def view_customer(session: Session, customer_id: int) -> sqlalchemy.orm.query.Query:
        ...
        entity = session.query(Customer).filter(Customer.customer_id == customer_id).first()
        if not entity:
            raise EntityNotFound(f"The customer \"{customer_id}\" does not exist in the database.")
        return entity

    @staticmethod
    def view_customer_orders(session: Session, customer_id: int) -> list[sqlalchemy.orm.query.Query]:
        """Returns all orders for a given customer."""
        entity = session.query(CustomerOrder).filter(CustomerOrder.customer_id == customer_id).all()
        if not entity:
    ...
    def view_order_grand_total(session: Session, order_id: int) -> int:
        ...
        for item in order_items:
            ...
            mod = session.query(ItemMod).filter(ItemMod.order_item_id == item.order_item_id).all()
            for m in mod:
    ...
    def view_order_customer(session, order_id) -> sqlalchemy.orm.query.Query:
        ...
        entity = session.query(CustomerOrder).filter(CustomerOrder.order_id == order_id).first()
        if not entity:
            raise EntityNotFound("The order does not have a customer.")
        return entity
    
    @staticmethod
    def view_order_items(session: Session, order_id: int) -> list[sqlalchemy.orm.query.Query]:
    ...
    @staticmethod
    def view_item_mods(session: Session, order_item_id: int) -> list[sqlalchemy.orm.query.Query]:
        """Returns all item modifications for a given order item."""
        entity = session.query(ItemMod).filter(ItemMod.order_item_id == order_item_id).all()
        if not entity:
